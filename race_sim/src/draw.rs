...
impl GameState {
    pub fn draw_perspective_road(&self, ctx: &mut Context) -> GameResult {
        ...
        for i in 0..10 {
            let y_start = road_top_y + (i as f32 * (road_bottom_y - road_top_y) / 10.0);
            let y_end = road_top_y + ((i + 1) as f32 * (road_bottom_y - road_top_y) / 10.0);
    ...
    pub fn draw_grass(&self, ctx: &mut Context) -> GameResult {
